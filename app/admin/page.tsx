...
export default function AdminDashboard() {
  const [settings, setSettings] = useState({
    appName: "PAL-TRUST", // 追加
    appSubtitle: "SURVEY", // 追加
    minStarsForGoogle: "4",
    aiReviewLength: "150",
    ...
    <div className="min-h-screen bg-[#F4F4F4] text-black font-sans lg:flex">
      ...
      <main className="flex-1 p-5 md:p-10 lg:p-16 max-w-[1000px] mx-auto w-full space-y-10">
        ...
        <section className="bg-white rounded-[2rem] border-3 border-black p-8 shadow-[8px_8px_0px_#000]">
          <h2 className="text-xl font-black mb-6 flex items-center gap-2 italic">
            <span className={`w-2 h-6 ${brandYellow} block border border-black`} /> 
            AI口コミ生成・返信 <span className="text-[10px] text-gray-400 ml-2 font-normal italic">AI ENGINE</span>
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
            ...
            <div>
              <label className="block text-[10px] font-black text-gray-400 mb-2 uppercase">口コミのテイスト</label>
              <select value={settings.aiReviewTaste} onChange={(e) => setSettings({...settings, aiReviewTaste: e.target.value})} className="w-full bg-[#F8F8F8] border-2 border-black p-4 rounded-xl font-bold outline-none">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-6 pb-20">
          ...
          <button onClick={handleSave} className="w-full md:w-auto order-1 md:order-2 bg-black text-white px-20 py-6 rounded-[2rem] font-black text-xl shadow-[8px_8px_0px_#F9C11C] active:scale-95 transition-all">設定を保存して反映</button>
        </div>
      </main>
    </div>
  );
